...
export async function retrieveAcrossAllKBs(query: string, perKb: number = 3) {
  ...
  const kbList = [
    { id: index.central.id, name: index.central.name, vectorStoreId: index.central.vectorStoreId },
    ...(index.userKbs ?? []).map((k) => ({ id: k.id, name: k.name, vectorStoreId: k.vectorStoreId })),
  ];
  ...
  // Sort by score if present (descending)
  all.sort((a, b) => (b.score ?? 0) - (a.score ?? 0));

  // Dedupe by identical text (simple)
